"""Rolling-horizon planning engine.

Implements the lot-sizing rules documented for Dynamics 365 F&O Planning
Optimization and for Infor LN Enterprise Planning, then replays them against a
demand history under rolling regeneration.

No F&O or LN system is run. These are reimplementations of documented
...
def fixed_order_quantity(net, Q):
    """Every order is exactly Q. Infor LN *Fixed Order Quantity*.

    "Enterprise Planning always makes the order quantity equal to the fixed
    order quantity", creating multiple orders where one is not enough.
